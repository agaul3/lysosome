# Medical School RPG — the first year (Milestones 1–11 and the first-year expansion)
...
**The first year.** The game now runs the whole first year of medical school, from the first day of classes (Monday 21 September) to the end-of-year celebration on the quad (Friday 28 May), as 28 story days in six blocks: Foundations; Musculoskeletal & Anatomy; Cardiovascular; Pulmonary & Renal; Endocrine & GI; Brain & Behavior. Each day opens with a title card; sleep in your bed at night to end it and see what the day brought. Check the Journal (Tab) for today's events and the map for where they are.
- **Classes:** twelve more lectures in Hall A and in Hall B of the Medical Education Center; histology, ECG and spirometry labs; the anatomy lab and practical in Anatomy Hall; standardized patients in the Clinical Skills Center; a small-group case; Clinical Immersion shifts in the Emergency Department, a family medicine clinic and on 4 West; a block exam every few weeks in the Testing Center (Pass or Honors, with a merit award); the White Coat Ceremony, the donor dedication, Research Day and the end-of-year OSCE. Walk to the room when it's time: from half an hour before, its check-in point offers the session. Every question becomes a flashcard on the day it's taught.
- **Life:** money (a monthly financial-aid disbursement, merit awards, paid tutoring shifts), food and drink with XP boosts at the Commons, the library's café, the Student Center's juice bar and the hospital's Food Court (off the corridor to the clinics); energy that food, rest and sleep restore; the Campus Store (clothing and study aids); the gym; the study group in the library; a skill tree of twenty perks; 46 achievements.
- **Clubs:** join up to three at the Club Fair on the quad or at the Office of Student Life: the Community Kitchen (serve supper on Harbor Street, a shuttle ride away), the Student-Run Free Clinic, the Surgery and Emergency Medicine interest groups, Medical Spanish, Journal Club and intramural soccer. Each meets on its own days and builds reputation.
- **Getting around:** the East Campus district along the Health Sciences Walk (the Medical Education Center, the Biomedical Library and the Student Center), Anatomy Hall north-west of the quad, and the campus shuttle.
...
- `world/interior/`, `world/med_ed/`, `world/library/`, `world/student_center/`, `world/anatomy/`, `world/community/`: the first year's buildings.
- `education/lectures/`, `education/questions/`, `education/activities/`: lecture scripts, question banks, and the year's labs, encounters, ceremonies and exams.
...
The other suites run the same way: `visual_motion`, `room_polish`, `seating`, `lecture`, `progression`, `knowledge`, `save`, `ui`, `acceptance` (the full spec §46 slice), `ambient` (courtyard life), `first_person` (the first-person view), `wardrobe` (characters, creator, clothing, closet, inventory), `flashcard` (computers and Anki), `cafe_exit`, `hospital` (University Hospital and the shadowing session), `emergency` (the Emergency Department and the campus ambulance), `ed_clearance` (scripted ED routes keep clear of furniture), `year` (the calendar, money, boosts, skills and achievements), `east_campus` (the district, shuttle, library and Medical Education Center), `student_life` (the Student Center, clubs and Anatomy Hall), `interior_clearance` (the new buildings' walkers), `curriculum` (the lectures), `activities` (labs, encounters, exams and ceremonies across the year) and `food_court` (the hospital Food Court), each as `res://tests/<name>_test.gd`. Run test scripts one at a time; they share a test save slot.
