...
The other suites run the same way: `visual_motion`, `room_polish`, `seating`, `lecture`, `progression`, `knowledge`, `save`, `ui`, `acceptance` (the full spec §46 slice), `ambient` (courtyard life) and `first_person` (the first-person view), each as `res://tests/<name>_test.gd`. Run test scripts one at a time; they share a test save slot.
