...
func _run() -> void:
	...
	check(dorm.hud.schedule_panel.summary_text().contains("Pharmacodynamics"), "Today's Schedule displays event")
	check(dorm.hud.calendar_panel.summary_text().contains("MANDATORY"), "Calendar shows mandatory event")
	dorm.hud.menu.show_page(8)
	check(dorm.hud.settings.is_visible_in_tree(), "Settings remains accessible in menu")
	dorm.hud.menu.show_page(2)
	...
	check(dorm.hud.schedule_panel.summary_text().to_lower().contains("late"), "Schedule retains lateness")
