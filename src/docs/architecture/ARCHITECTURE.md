...
`ui/computer/desktop.gd` owns the Windows- and macOS-style shells.
- **Rendering:** it renders the OS into a SubViewport (1024×576 or 1024×640 logical, rendered up to 2× sharper) that it draws onto the device's world screen quad (`Dorm.pc_screen`, `Laptop.screen`).
- **Camera:** a `LectureCamera` instance eases to `view_pose()` square to the screen. `LectureCamera` accepts perspective as well as orthographic sources and has per-use `target_fov`, `duration`, `arc_height` and `focus_distance`.
- **Input:** `_input` ray-traces mouse events onto the quad (`screen_point()`) and forwards them and key events with `SubViewport.push_input`. Anki shortcuts are checked first, except while a text field has focus.
