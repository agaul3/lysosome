...
- The OS is shown on the device's own screen in the world; the camera eases in to a seated eye point with the surroundings still visible, and the mouse works directly on the screen.
- Double-click **Anki** on the Windows desktop (or use the taskbar or Start menu), or click it in the Mac Dock. **Esc** closes the computer. Player movement and interaction are held while it is open; first-person mouse capture is released. The world and academic clock continue running. Lecture shortcuts do not consume computer input.
...
The light Anki-style app follows desktop Anki's layout (toolbar, deck tree, overview, reviewer bottom bar, Add window, Browse with is:/deck:/tag: search, Stats, Deck Options): Decks / Add / Browse / Stats / Sync, blue new counts, red learning counts, green review counts, a question-first study screen, separate answer and explanation, and four interval-labelled rating buttons. Long content scrolls while recall buttons remain visible. Space reveals; after revealing, Space/Enter chooses Good and 1–4 chooses Again/Hard/Good/Easy. Ratings are rejected before reveal and duplicate submissions cannot change progress.

The starter deck is `Medicine::Pharmacology::Pharmacodynamics`. Since Milestone 11, bank cards are filed one deck per discipline and topic (`Medicine::<discipline>::<topic>`), so the shadowing etiquette checks form `Medicine::Clinical Skills::Hospital Orientation`. The pharmacology deck adapts 13 standalone records from the shared QuestionBank at runtime, hides multiple-choice distractors, and retains the exact correct answer, explanation, objective and source ID. The graph-dependent `pd_viz_competitive_01` is excluded because its prompt needs the lecture visualization. Bank content is inspectable but is edited at its authoritative source, not overwritten in a personal save.
